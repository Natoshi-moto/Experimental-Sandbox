...
4. ~~Hermes Agent has no native Anthropic/Claude API provider support~~ — **this was wrong, caught during the next round's sanity check.** Fetched `configuration.md` and `cli-config.yaml.example` directly from the repo (not a summarized secondary source): `provider: "anthropic"` is a first-class named option (`ANTHROPIC_API_KEY`), with its own per-model timeout tuning, alongside a documented "native Anthropic" wire transport distinct from "OpenAI-wire." Doesn't change this prototype's decision — operator confirmed DeepSeek + local only, Anthropic support existing or not was never the deciding factor — but the earlier claim was a real error, not just imprecise, and it was caught by re-fetching primary source instead of trusting a prior summary. Kept here as a reminder that source-of-record beats even this document's own prior round.

5. ~~The `config.yaml` schema shown in SPEC.md matches the current shipped version~~ — **resolved: fetched `website/docs/user-guide/configuration.md`, `website/docs/user-guide/cli.md`, and `cli-config.yaml.example` directly from `NousResearch/hermes-agent` via the GitHub API (raw file content, not a docs-site summary).** Real, confirmed details that changed the spec:
   - Secrets use `key_env: <ENV_VAR_NAME>` pointing at a variable name, not an inline `api_key: ${VAR}` substitution (both syntaxes exist in different contexts — `key_env` is the documented pattern for named custom providers, which is what this prototype uses).
   - `provider: "custom"` has aliases: `"ollama"`, `"vllm"`, `"llamacpp"` all map to the same thing. DeepSeek has no named shortcut (not in the ~20-item named-provider list) and must use `provider: custom` with an explicit `base_url`.
   - There is no `hermes doctor` command (an earlier round's guess). The real config health-check is `hermes config check` (flags missing options after updates) and `hermes config migrate` (interactively adds them).
   - Six terminal backends confirmed with exact names: `local | docker | ssh | modal | daytona | singularity` — matches the "six backends" figure from earlier rounds exactly.
   - `HERMES_YOLO_MODE` / `hermes --yolo` / `/yolo` exists — an auto-approve-everything mode, visibly flagged in the CLI's own status bar when active (`⚠ YOLO`). **Decision: leave this off for the first sweep.** Confirmed with the operator, not defaulted silently.
   - `credential_pool_strategies` (round_robin, least_used, etc.) exists but wasn't found in the files actually fetched this round — treat that specific field name as still unconfirmed until seen directly, downgraded from "verified" to "unconfirmed" here.
...
8b. **Which local model to assign as the auxiliary/local provider — resolved with a live benchmark, not a guess.** `qwable-3.6-27b-abliterated` was the "sharpest" abliterated model by parameter count, but is 27GB against a GPU with 8.9GB free VRAM at the time of testing — CPU-fallback risk on a system that was simultaneously at 406MB free system RAM and a full 8GB swap. Not run, for stability reasons. Two candidates that fit fully in free VRAM were actually run (`ollama run --verbose`, real numbers, not estimated):

    | Model | Load time | Generation speed | Context |
    |---|---|---|---|
    | `obliterated-gemma:q4` | 20.7s | 18.66 tok/s | 131K |
    | `hf.co/KakTakOne/Huihui-gemma-4-12B-coder-fable5-composer2.5-v1-abliterated-GGUF:Q4_K_M` | 21.0s | 22.59 tok/s | 262K |

    **Decision: `Huihui-gemma-4-12B-coder...abliterated` — faster, larger context, and coder-tuned.** Re-run this same benchmark after install if the machine's memory pressure has changed materially (see item 7's ollama check) — these numbers were taken under real, fairly loaded conditions (~10 concurrent Claude Code sessions running), so they're a realistic floor, not a best case.
